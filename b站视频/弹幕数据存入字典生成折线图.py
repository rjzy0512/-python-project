...
# TODO 使用for 循环遍历range()函数生成的0-24的数字
...
for xunhuan in subtitlesDict:

    # TODO 使用split()分隔字典的键获取第一项，赋值给变量start_key
    start_key = xunhuan.split("-")[0]

    # TODO 使用split()分隔字典的键获取第二项，赋值给变量end_key
    end_key = xunhuan.split("-")[1]
    ...
    for item in timeList:
        ...
        if int(start_key) <= item <= int(end_key):
            # TODO 将字典中键所对应的值累加
            subtitlesDict[xunhuan] += 1

# TODO 使用Line()创建Line对象，赋值给line=LI
line = Line()
